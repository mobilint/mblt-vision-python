...
from ..wrapper import CoreMode, MBLT_Engine, download_hub_artifact
...
class SAM2HieraLarge(MBLT_Engine):
    ...
    Args:
        ...
        target_device: NPU board identifier shared by both MXQ artifacts.
            ...
            validated on.
        revision: Hugging Face Hub revision for all three artifacts.
        ...
        device: Torch device for the host-side prompt encoding (a handful of
            small embeddings/lookups). The two MXQ artifacts always run on
            the NPU regardless of this setting.
    """
    ...
    def __init__(
        ...
        self._closed = False
        self._encoder_backend: MobilintNPUBackend | None = None
        self._decoder_backend: MobilintNPUBackend | None = None
        ...
        for label, path in (
            ...
            if path and Path(path).suffix.lower() != ".mxq":
                raise ValueError(f"Explicit {label} must end in '.mxq', got {path!r}.")

        resolved_target_device = normalize_target_device(target_device or "aries-rb")
    ...
    def preprocess(self, x: Any, **kwargs: Any) -> np.ndarray:
        """Resize/pad/normalize an image into the encoder MXQ's NHWC input."""
    ...
    def predict_preprocessed(
        ...
        encoder_outputs = encoder_backend(encoder_feed)
        feature_maps = fpn_from_runtime(encoder_outputs, self.device)
        ...
        decoder_outputs = classify_decoder_outputs(decoder_backend(decoder_feed))
    ...
    def _require_encoder_backend(self) -> MobilintNPUBackend:
