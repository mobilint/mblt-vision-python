...
def test_mxq_classification_runs_with_shared_npu_options(
    npu_params: NpuParams, synthetic_image_path
) -> None:
    """Load a representative MXQ model using the shared NPU test options."""

    model = MBLT_Engine(model_cls="resnet50", **npu_params.base)
    try:
        result = model.postprocess(model(model.preprocess(str(synthetic_image_path))))
